...
const FROM_EMAIL = "ertesito@kolcsonadlak.hu";
const FROM_NAME = "Kölcsönadlak.hu";
const REPLY_TO = "kulovanyi.kornel@gmail.com";
...
module.exports = async (req, res) => {
    // CORS és UTF-8 fejléc beállítások
    ...
    try {
        ...
        if (type === "rental_request") {
            ...
            subject = `🎉 [Kölcsönadlak - Bérlés Elfogadva] A bérbeadó elfogadta a kérelmedet: ${data.item_title || "Eszköz"}`;
            ...
            return res.status(400).json({ error: "Érvénytelen értesítés típus" });
        }
        ...
        // 1. Címzettnek küldés a Gmail SMTP-n
        const info = await transporter.sendMail(mailOptions);

        // 2. Admin másolat a külön admin címre
        if (ADMIN_EMAIL && ADMIN_EMAIL !== to) {
            try {
                ...
                console.warn("Admin copy warning:", admErr);
            }
        }
